...
public class Chain<Object> extends ArrayList<Object> {
	...
	public Chain(Collection<Object> col) {
		super(col);
	}
	...
	public Object getPrevious(int index) {
		if (index == 0)
			throw new IndexOutOfBoundsException("trying to get previous of the first.");
		return get(index - 1);
	}
	...
	public Object getNext(int index) {
		if (index == size() - 1)
			throw new IndexOutOfBoundsException("trying to get next of the last.");
		return get(index + 1);
	}

	public Object getNext(Object o) {
		// BEN J'ai un souci ici pour mes getNext et previous, qui lance des indexOf. il semble que le indexof fasse un
		// equals(),
		// donc lorsqu'un polygon a deux points consecutifs egaux, le getNext ou previous bloque.
		// Comment faire pour ne chercher que les objets egaux de reference (et non de valeurs) dans une collection? Je
		// crois que
	...
	public int indexOf(java.lang.Object o) {
		...
		if (index == -1)
			throw new RuntimeException("The given " + o.getClass().getName() + " doesn't exist in this "
